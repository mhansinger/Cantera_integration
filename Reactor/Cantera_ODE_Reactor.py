...
class Cantera_ODE_Reactor(object):
    def __init__(self):
        ...
        self.Reactor_database_org = None
        print('Time step is: %f' % self.dt)

    def read_data(self,name,path='/home/max/HDD2_Data/OF4_Simulations/ANN_Lu19_data/Reactor_database'):

        self.Reactor_data_path = join(path,name)
        self.Reactor_database_org=pd.read_hdf(self.Reactor_data_path)

        print('These are the data features:')
        print(self.Reactor_database_org.columns)

    def read_data_dd(self, name,path='/home/max/HDD2_Data/OF4_Simulations/ANN_Lu19_data/Reactor_database'):

        self.Reactor_data_path = join(path, name)
        self.Reactor_database_org=dd.read_hdf(self.Reactor_data_path, key='Reactor_raw_data')
    ...
    def set_tables(self,name):
        ...
        # reaction rate [1/s]
        self.columns_RR = ['RR_'+n for n in self.species_names]      # now equivalent to OpenFOAM
        self.columns_out = self.columns_out + self.columns_out_after +self.columns_RR

        self.columns_out.append('f_Bilger')

        self.len_dataset = len(self.Reactor_database_org)
    ...
    def loop_ODE(self,remove_T_below,steps):
        ...
        for idx_fullset, this_set in tqdm(self.Reactor_database_org.iterrows()):
            #print('Row number: ',idx_fullset)

            #this_set = self.Reactor_database_org.iloc[row].calculate()      # calculate only if NOT dask!
            ...
            if this_T > remove_T_below:
                ...
                print('Removed! T is at: %f' % this_T)
