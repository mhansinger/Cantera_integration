...
for index, yF in enumerate(yFuel):
    ...
    P_vector = np.ones(len(f.grid)) * f.P
    f_Bilger_vector = np.ones(len(f.grid)) * mixtureFractionBilger

    column_names = gas.species_names + ['T','p','f_Bilger']
    data_np =  np.concatenate((f.Y,[f.T,P_vector,f_Bilger_vector]),axis=0)

    data_df = pd.DataFrame(data=data_np.T ,columns=column_names)
